...
// All admin alerts (new order, new customer) go here.
const ADMIN_EMAIL = 'foodfi925@gmail.com'
...
// ── Low-stock alert → foodfi925@gmail.com ────────────────────────
export async function sendLowStockAlert({ itemName, stock, branchName }) {
  const out = stock === 0
  const at = branchName ? ` (${branchName})` : ''
  const subject = out ? `❌ OUT OF STOCK: ${itemName}` : `⚠️ Low stock: ${itemName} — ${stock} left`
  const accent = out ? '#dc2626' : '#d97706'
  const html = `
    <div style="font-family:Arial,sans-serif;max-width:520px;margin:0 auto;padding:22px;border:1px solid #eee;border-radius:12px;">
      <h2 style="color:${accent};margin:0 0 10px;">${out ? '❌ Item Out of Stock' : '⚠️ Low Stock Alert'}</h2>
      <p style="font-size:15px;color:#1f2937;margin:0 0 6px;line-height:1.5;">
        <b>${itemName}</b>${at} — ${out
          ? 'stock ab <b>0</b> hai. Customer ise order nahi kar payega.'
          : `sirf <b>${stock}</b> bacha hai.`}
      </p>
      <p style="font-size:13.5px;color:#6b7280;margin:0 0 16px;">Refill karke Admin me stock badha dein / Reset Stock dabayein.</p>
      <a href="https://foodfi.in/admin" style="display:inline-block;background:#e85d04;color:#fff;text-decoration:none;padding:10px 18px;border-radius:8px;font-weight:700;">Open Admin</a>
      <p style="font-size:11px;color:#9ca3af;margin:16px 0 0;">FoodFi Kitchen · automated stock alert</p>
    </div>`
...
// ── New Order Alert (to admin) — full details ───────────────────
export async function sendNewOrderAdminEmail({
  orderNumber, customerName, customerPhone, items = [],
  subtotal, discountAmount = 0, deliveryCharge = 0, total, deliveryAddress
}) {
  const now = new Date().toLocaleString('en-IN', { timeZone: 'Asia/Kolkata', hour12: true })
  ...
  const discountRow = discountAmount > 0
    ? `<tr><td colspan="2" style="padding:6px 4px;font-size:13px;color:#16a34a;">Discount</td>
        <td style="text-align:right;font-size:13px;color:#16a34a;">−₹${Math.round(discountAmount)}</td></tr>` : ''
  ...
  const html = `
    <div style="font-family:Arial,sans-serif;max-width:480px;margin:0 auto;padding:20px;background:#fff;">
      ...
      <div style="background:#fff7ed;border-left:4px solid #e85d04;border-radius:10px;padding:14px 18px;margin-bottom:18px;">
        <p style="margin:0;font-size:16px;font-weight:700;color:#c2410c;">🔔 Naya Order Aaya — #${orderNumber}</p>
        <p style="margin:4px 0 0;font-size:12px;color:#6b7280;">${now} (IST)</p>
      </div>

      <table style="width:100%;border-collapse:collapse;margin-bottom:8px;">
        <tr style="border-bottom:1px solid #f3f4f6;">
          <td style="padding:9px 4px;font-size:13px;color:#6b7280;width:32%;">👤 Customer</td>
          <td style="padding:9px 4px;font-size:14px;font-weight:700;color:#1f2937;">${customerName || '—'}</td>
        </tr>
        <tr style="border-bottom:1px solid #f3f4f6;">
          <td style="padding:9px 4px;font-size:13px;color:#6b7280;">📱 Phone</td>
          <td style="padding:9px 4px;font-size:13px;color:#1f2937;">${customerPhone || '—'}</td>
        </tr>
        <tr>
          <td style="padding:9px 4px;font-size:13px;color:#6b7280;vertical-align:top;">📍 Location</td>
          <td style="padding:9px 4px;font-size:13px;color:#1f2937;">${deliveryAddress || '—'}</td>
        </tr>
      </table>
      ...
      <table style="width:100%;border-collapse:collapse;margin:14px 0;">
        ...
        <tfoot>
          ...
          <tr style="border-top:2px solid #f3f4f6;">
            <td colspan="2" style="padding:10px 4px;font-size:15px;font-weight:700;color:#1f2937;">TOTAL</td>
            <td style="text-align:right;font-size:15px;font-weight:700;color:#e85d04;">₹${Math.round(total)}</td>
          </tr>
        </tfoot>
      </table>
  ...
  await sendEmail({ to: ADMIN_EMAIL, subject: `🔔 Naya Order #${orderNumber} — ${customerName || 'Customer'} (₹${Math.round(total)})`, html })
}
