...
async function loyaltyConfig(sql) {
  try {
    ...
    const [s] = await sql`SELECT loyalty_enabled, loyalty_threshold, loyalty_reward, loyalty_started_at FROM kitchen_settings WHERE id = 1`
    ...
    return { threshold, reward, startedAt }
  } catch { return null }
}
