...
export async function getDeliveryQuote(distanceKm, subtotal = 0) {
  ...
  if (!row) return { baseCharge: 50, freeDeliveryMin: null, freeDelivery: false, deliveryCharge: 50 }
  const extraKm = Math.max(0, distanceKm - parseFloat(row.min_km))
  ...
  const freeDelivery = freeDeliveryMin != null && freeDeliveryMin > 0 && subtotal >= freeDeliveryMin
  return { baseCharge, freeDeliveryMin, freeDelivery, deliveryCharge: freeDelivery ? 0 : baseCharge }
}
