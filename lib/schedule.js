...
export async function checkAndUpdateKitchenSchedule() {
  try {
    ...
    const [settings] = await sql`
      SELECT is_open, auto_schedule, open_time, close_time, force_closed
      FROM kitchen_settings WHERE id = 1
    `
    if (!settings || !settings.auto_schedule) return

    // Admin ne manually CLOSE kiya hai — schedule override nahi karega
    if (settings.force_closed) return
