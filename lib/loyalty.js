...
export async function reconcileLoyalty(sql, userId) {
  ...
  try {
    ...
    const given = e?.c || 0
    if (expected <= given) return   // nothing newly earned

    // Keep at most ONE reward pending at a time — the customer redeems it before
    // earning the next (so the card shows ₹{reward}, never a stacked ₹{2×reward}).
    ...
    // Attach the new reward to a delivered order not already used as a reward
