...
// channelId/sound default to the general channel. For the LOUD order alarm,
// pass { channelId: 'foodfi_orders', sound: 'order_alarm' } — but ONLY once the
// app version that defines that channel has shipped (Android drops/relocates a
// notification whose channel doesn't exist on the device).
export async function sendFcmToTokens(tokens, { title, body, orderId, tag, channelId = 'foodfi_default', sound = 'default' }) {
  const sa = getServiceAccount()
