...
export async function sendPushToUser(userId, { title, body, url = '/', tag, requireInteraction = false }, role = 'customer') {
  try {
    initVapid()
    const sql = getDb()
...
export async function sendPushToRole(role, { title, body, url = '/', tag, requireInteraction = false }) {
  try {
    ...
    await Promise.all(subs.map(async s => {
      // Save to notifications DB for each user
      await saveNotification(sql, s.user_id, role, { title, body, url, tag })
      return webpush.sendNotification(s.subscription, payload).catch(async err => {
        if (err.statusCode === 410 || err.statusCode === 404) {
          await sql`DELETE FROM push_subscriptions WHERE id = ${s.id}`.catch(() => {})
        }
      })
